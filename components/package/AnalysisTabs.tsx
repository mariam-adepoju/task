...
export function AnalysisTabs({ data }: AnalysisTabsProps) {
    ...
    const triggerStyles = cn(
        ...
        "rounded-none h-full px-1.5 md:px-3 py-1.5 text-muted-foreground text-sm rounded-lg font-medium transition-all"
    );

    return (
        <Tabs defaultValue="overview" className="w-full gap-0">
            <div className="border px-4 py-2 bg-background">
                <TabsList className="bg-transparent h-14 gap-2 sm:gap-6">
                    {tabs.map((tab) => (
                        <TabsTrigger
                            key={tab.id}
                            value={tab.id}
